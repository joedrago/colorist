    ...
    if ((useCCMM && !transform->ccmmReady) || (!useCCMM && !transform->lcmsReady)) {
        ...
        switch (transform->tonemap) {
            case CL_TONEMAP_AUTO:
                transform->tonemapEnabled = (luminanceScale > 1.0f) ? clTrue : clFalse;
                break;
            case CL_TONEMAP_ON:
...
static void transformFloatToFloat(struct clContext * C, struct clTransform * transform, clBool useCCMM, uint8_t * srcPixels, int srcPixelBytes, uint8_t * dstPixels, int dstPixelBytes, int pixelCount)
{
    ...
    for (i = 0; i < pixelCount; ++i) {
        ...
        // Convert to xyY
        clTransformXYZToXYY(C, xyY, XYZ, transform->whitePointX, transform->whitePointY); // TODO: use actual src whitepoint

        // Luminance scale
        xyY[2] *= (useCCMM) ? transform->ccmmLuminanceScale : transform->lcmsLuminanceScale;
